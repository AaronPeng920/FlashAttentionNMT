...
def attention(query, key, value, mask=None, dropout=None):
    ...
    if mask is not None:
        scores = scores.masked_fill(mask == 0, -1e4)        # set -1e4 for dit scale bf16
...
def flash_attention_v1(query, key, value, mask=None, dropout=None):
    """params
        query: [B, H, N, C]
        key: [B, H, N, C]
        value: [B, H, N, C]
    ...
    query = query.transpose(1, 2).view(-1, H, C)
    key = key.transpose(1, 2).view(-1, H, C)
    value = value.transpose(1, 2).view(-1, H, C)
    
    cu_seqlens_q = torch.arange(0, (B+1)*q_L, q_L, dtype=torch.int32, device=DEVICE)
    cu_seqlens_kv = torch.arange(0, (B+1)*kv_L, kv_L, dtype=torch.int32, device=DEVICE)
    ...
    # [B*L, H, C]
    out, softmax_lse, S_dmask = flash_attn_unpadded_func(query, key, value, cu_seqlens_q=cu_seqlens_q, 
                                                   cu_seqlens_k=cu_seqlens_kv, max_seqlen_q=int(q_L), max_seqlen_k=int(kv_L), 
                                                   dropout_p=dropout, softmax_scale=None, causal=casual, return_attn_probs=True)
    out = out.contiguous().view(B, H, q_L, C) 

    return out, softmax_lse

def flash_attention_v2(query, key, value, mask=None, dropout=None):
    """params
        query: [B, H, N, C]
        key: [B, H, N, C]
        value: [B, H, N, C]
    ...
    # [B*L, H, C]
    out = flash_attn_func(query, key, value, dropout_p=dropout, softmax_scale=None, causal=casual, window_size=(-1, -1))
    out = out.contiguous().view(B, H, q_L, C) 
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        x, self.attn = flash_attention_v1(query, key, value, mask=mask, dropout=self.dropoutrate)
