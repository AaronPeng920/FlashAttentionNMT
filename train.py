...
def run_epoch(data, model, loss_compute):
    ...
    for batch in tqdm(data):
        ...
        # batch.src = batch.src.half()
        # batch.trg = batch.trg.half()
        # batch.src_mask = batch.src_mask.half()
        # batch.trg_mask = batch.trg_mask.half()
        ...
        total_loss += loss.float()          # add .float()
        # print(total_loss)
        total_tokens += batch.ntokens.float()       # add .float()
        # time_end = time.time()
        # if is_train:
        #     logging.info("Epoch: {}, Batch: {}, time: {}".format(epoch, batch_idx, time_end - time_start))
...
def train(train_data, dev_data, model, model_par, criterion, optimizer):
    ...
    for epoch in range(1, config.epoch_num + 1):
        ...
        end_train = time.time()
        logging.info("Epoch: {}, loss: {}, time: {}".format(epoch, train_loss, end_train-start_train))
        # 模型验证
        model.eval()
        dev_loss = run_epoch(dev_data, model_par, LossCompute(model.generator, criterion, optimizer))
